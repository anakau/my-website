...
// Prepare country list (still used for flags on existing candles)
...
export default function Home() {
  ...
  const handleWorldClick = async e => {
    ...
    if (!error && data?.length) {
      ...
      setCandles(prev => {
        setModal({
          open:true,index:idx,id:newRow.id,
          text:'',country:'',
  ...
  return <>
    ...
    {/* info popover */}
    {showInfo && <div style={{position:'fixed',inset:0,zIndex:900}}>
    ...
    <div ref={worldRef}
      ...
      <div style={{width:3000,height:2000,position:'relative'}}>
        {candles.map((c,i)=><div key={c.id}
          ...
          {c.country_code&&<div style={{
            fontSize:18,
            marginTop:0,  /* very tight under candle */
            lineHeight:1
          }}>
    ...
    {/* ghost preview */}
    {isPlacing && dragPos && <img
      src="/candle.gif" alt="Ghost Candle"
      style={{
    ...
    {/* letter modal bubble below flag */}
    {modal.open && <>
      {/* backdrop */}
      <div onClick={()=>setModal({open:false,index:null,id:null,text:'',country:'',x:0,y:0})}
      ...
      <div onClick={e=>e.stopPropagation()}
        style={{
          ...
          background:'#f2f2f2',borderRadius:12,
        ...
        {/* emoji picker input */}
        <input type="text" placeholder="😀 — open emoji picker"
          value={modal.country}
