...
export default function Home() {
  ...
  const [modal, setModal] = useState({
    ...
    selectedEmoji: '', showEmojiPicker: false
  })
  ...
  // 6) submit letter+flag+emoji
  const submitModal = async () => {
    const { index, id, text, country, selectedEmoji } = modal
    setCandles(prev => {
      const cp = [...prev]
      cp[index].note         = text
  ...
  return (
    <>
      ...
      {/* Ghost preview */}
      {isPlacing&&dragPos&&(
        <img src="/candle.gif" alt="Ghost Candle"
          style={{
      ...
      {modal.open&&(
        <>
          ...
          <div onClick={e=>e.stopPropagation()}
            style={{
              position:'fixed',left:modal.x,top:modal.y+24,
          ...
          >
            ...
            <div style={{ marginBottom:12 }}>
              <button type="button"
                onClick={()=>setModal(m=>({
                  ...m,showEmojiPicker:!m.showEmojiPicker
                }))}
                style={{
                  padding:'6px 12px',
                  border:'1px solid #ddd',borderRadius:4,
              ...
              {modal.showEmojiPicker&&(
                <div style={{
                  position:'fixed',left:modal.x,top:modal.y+60,
